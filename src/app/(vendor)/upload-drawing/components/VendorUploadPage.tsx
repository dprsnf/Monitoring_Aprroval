...
type ResubmitDoc = {
  ...
  approvals?: { notes?: string }[];
  reviewedBy?: { name?: string; division?: string };
  status?: "returnForCorrection" | "approvedWithNotes" | string;
  progress?: string[];
...
interface VendorUploadPageProps {
  initialResubmitDocs: ResubmitDoc[];
}
...
export default function VendorUploadPage({
  ...
  const [resubmitDocs, setResubmitDocs] =
    useState<ResubmitDoc[]>(initialResubmitDocs);
  ...
  const getDocumentNotes = (doc: ResubmitDoc) => {
                        ...
                        {doc.progress && doc.progress.length > 1 && (
                          <details className="mt-4">
                            <summary className="cursor-pointer text-sm font-semibold text-gray-600 hover:text-gray-800">
                              Lihat Riwayat Review ({doc.progress.length}{" "}
                              aktivitas)
                            </summary>
                          ...
                          <input
                            ...
                            onChange={async (e) => {
                              ...
                              const confirmUpload = confirm(
                                `Upload revisi untuk dokumen "${doc.name}"?`,
                              );
                              if (!confirmUpload) {
                                e.target.value = "";
                                return;
