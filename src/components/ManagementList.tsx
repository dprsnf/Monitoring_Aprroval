import { Eye, CheckCircle, XCircle, Download, User, Calendar, FileText, Clock, Send, Ban } from "lucide-react";
...
export default function ManagementList({
    ...
    const canTakeAction = (document: Document) => {
        ...
        if (currentUserRole === "Manager") {
            return document.status === Status.submitted || document.status === Status.returnForCorrection;
        }
        return false;
    };
    ...
    return (
        <div className="space-y-3 sm:space-y-4">
            {data.map((document) => (
                <Card key={document.id} className="shadow-xl bg-white/95 backdrop-blur-sm border border-white/30 hover:shadow-2xl transition-all duration-200">
                    <CardContent className="p-4 sm:p-6">
                        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
                            <div className="flex-1">
                                <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-3 mb-3">
                                    <h3 className="text-base sm:text-lg font-semibold text-gray-900">{document.name}</h3>
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2 sm:gap-4 text-xs sm:text-sm text-gray-600 mb-3">
                                    <div className="flex items-center gap-2">
                                        ...
                                        <span><strong>Vendor:</strong> {document.submittedBy.name}</span>
                                    </div>
                                    <div className="flex items-center gap-2">
                                        <Calendar className="w-3 h-3 sm:w-4 sm:h-4 text-[#125d72]" />
                                        <span><strong>Submit:</strong> {new Date(document.createdAt).toLocaleDateString('id-ID')}</span>
                                    </div>
                                    <div className="flex items-center gap-2">
                                ...
                                {document.contract && (
                                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-4 text-xs sm:text-sm text-gray-600 mb-3">
                                        <div className="flex items-center gap-2">
                                            ...
                                            <span><strong>Contract:</strong> {document.contract.contractNumber}</span>
                                        </div>
                                        <div className="flex items-center gap-2">
                                            <Calendar className="w-3 h-3 sm:w-4 sm:h-4 text-blue-600" />
                                            <span><strong>Contract Date:</strong> {new Date(document.contract.contractDate).toLocaleDateString('id-ID')}</span>
                                        </div>
                                    </div>
                                )}
                                ...
                                {document.overallDeadline && (
                                    <div className="mb-3">
                                        <div className="flex items-center gap-2 text-xs sm:text-sm text-red-600 font-medium">
                                            ...
                                            <span>Deadline: {new Date(document.overallDeadline).toLocaleDateString('id-ID')}</span>
                                        </div>
                                    </div>
                                )}
