...
export default function Header({ currentUser, title, backHref = "/", backLabel }: HeaderProps) {
  return (
        ...
        <div className="flex items-center justify-between h-14 sm:h-16">
          <div className="flex items-center gap-1 sm:gap-2 lg:gap-4 min-w-0 flex-1">
            ...
            <div className="min-w-0 flex-1">
              {/* use custom title if provided; fall back to existing responsive titles */}
              {title ? (
                <h1 className="text-xs sm:text-sm lg:text-xl font-semibold text-white truncate">{title}</h1>
              ) : (
                <h1 className="text-xs sm:text-sm lg:text-xl font-semibold text-white truncate">
                  <span className="block sm:hidden">Doc Review</span>
                  <span className="hidden sm:block lg:hidden">Document Review</span>
                  <span className="hidden lg:block">Document Review System</span>
                </h1>
              )}
            </div>
          </div>
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button className="bg-[#efe62f] hover:bg-[#125d72] text-gray-900 hover:text-white transition-all duration-200 px-2 sm:px-3 py-1.5 sm:py-2 rounded-lg">
                  <span className="text-xs sm:text-sm font-medium mr-1">{currentUser.name.split(" ")[0]}</span>
              ...
              >
                <DropdownMenuLabel className="bg-gradient-to-r from-[#125d72]/10 to-[#14a2ba]/10 rounded-t-lg">
                  <div className="py-1">
                    ...
                    <p className="text-xs text-[#14a2ba]">{currentUser.role} - PLN</p>
